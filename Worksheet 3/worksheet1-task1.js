...
function getCoord(result){
	...
	let dispArea = document.getElementById("map");

	try {
		dispArea.innerHTML = "Location on map"

		let latit = result.results[0].geometry.lat;
		let longit = result.results[0].geometry.lng;
		let coordinates = [longit,latit];
	  ...
	  marker.addTo(map);
		...
		dispArea.innerHTML = "Location Unknown"
	}
...
function viewStudent(index)
{
	...
	let homeAddress = JSON.stringify(selectedStudent.address);
	let url = "https://api.opencagedata.com/geocode/v1/json?key=" + opencageKey + "&q=" + homeAddress + "&jsonp=getCoord";
...
let opencageKey = "59b4c8149ac74a9a8c921d9bf896de92";
